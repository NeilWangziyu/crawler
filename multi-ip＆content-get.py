...
def get_content(rawID,sheet,proxies):
    global  counter
    global patentID
    ID1 = rawID[:12]
    ID2 = rawID[12]
    detailurl = "http://so.baiten.cn/detail/patentdetail/63/CN" + ID1 + "." + ID2 + "/25"
    print("processing:" + detailurl)
    print(proxies)
    print("counter:",counter)
    detailhtml = requests.get(detailurl,proxies=proxies, timeout = 3)
    ...
    sheet.write(counter, 9, detailurl)

def pre_get_content():
    global counter
    global patentID
    if (len(patentID) != 0):
        if (len(patentID[0]) == 14):
            ...
            while (True):
                try:
                    rawID = patentID[0]
                    get_content(rawID, sheet, proxies)
                    counter = counter + 1
                    del patentID[0]
            ...
            print("wrongID:", patentID[0])
            del patentID[0]
    else:
        pass

def run_pre_get_content():
    global counter
    global patentID
    lock.acquire()
    try:
...
if __name__ == "__main__":
    url = 'http://ttvp.daxiangip.com/ip/?tid=555201805942010&num=10'
    ipp = IP_pool(url)
    global counter
    global patentID
    counter = 1
    ...
    while(len(patentID) != 0):
        for i in range(1,21):
            name = str(i)
            name = threading.Thread(target=pre_get_content, args=())
            name.start()
            name.join()
